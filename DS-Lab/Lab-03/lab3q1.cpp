...
int main(){
	...
	for(int test=0;test<8;test++){
		cout<<endl<<" |"<<p[test]<<" \t |"<<q[test]<<" \t |"<<r[test]<<" \t |"<<compP[test]<<" \t |"<<compQ[test]<<" \t |"
				  <<ImpliesTo(LogicalOr(LogicalAnd(p[test],compQ[test]),r[test]),LogicalOr(compP[test],r[test]))<<" \t\t\t\t |"
				  <<DoubleArrow(LogicalOr(LogicalAnd(p[test],compQ[test]),r[test]),LogicalOr(compP[test],r[test]));
	}
}
